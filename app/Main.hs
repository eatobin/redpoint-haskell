...
myMainLoop :: IO MyState -> IO ()
myMainLoop nextStateIO = do
  nextState <- nextStateIO
  if map DC.toLower (quit nextState) == "q"
    then do
      ...
      putStrLn "Thanks! Bye...\n"
    else myMainLoop (myStateAskContinue (myStatePrintResults (myStateUpdateAndRunNewYear (return nextState))))

main :: IO ()
main =
  do
    let firstState = DM.fromJust (myStateJsonStringToMyState hawksJson)
    myMainLoop (myStateAskContinue (myStatePrintResults (return firstState)))
